...
public class alipaySweepPaymentcontroller {
     ...
     */  
     ...
     public String test_trade_pay(String orderNumber,String subject,String totalAmount,String authCode ) {
         ...
         // 商品明细列表，需填写购买商品详细信息，
         List<OrderProduct> OrderProduct = orderProductService.listorderNumberOrderProduct(outTradeNo);
         List<GoodsDetail> goodsDetailList = new ArrayList<GoodsDetail>();
         for(int i=0;i<OrderProduct.size();i++) {
        	 goodsDetailList.add(GoodsDetail.newInstance(OrderProduct.get(i).getProductId().toString(),OrderProduct.get(i).getProductId().toString() , (OrderProduct.get(i).getSalePrice().multiply(new BigDecimal(100)).longValue()), OrderProduct.get(i).getProductCount()));
         }
		/*
