...
public class WXPay {
     ...
     * 向 Map 中添加 appid、mch_id、nonce_str、sign_type、sign <br>
     * 该方法适用于商户的退款
     * 详细的看官方接口文档，懒得看我总结的  官方微信退款接口文档:https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_4
     * @param reqData
     * @return
     * @throws Exception
     */
    public Map<String, String> cateringRefund(Map<String, String> reqData) throws Exception {
        ...
        /*微信订单号   transaction_id        二选一                     String(32)      微信生成的订单号，在支付通知中有返回
                        商户订单号   out_trade_no          必填                        String(32)      商户系统内部订单号，要求32个字符内，只能是数字、大小写字母_-|*@ ，且在同一个商户号下唯一。
                        商户退款单号  out_refund_no        必填                        String(64)      商户系统内部的退款单号，商户系统内部唯一，只能是数字、大小写字母_-|*@ ，同一退款单号多次请求只退一笔。
                        订单金额    total_fee              必填                        Int 100         订单总金额，单位为分，只能为整数，详见支付金额
                        退款金额    refund_fee             必填                        Int 100         退款总金额，订单总金额，单位为分，只能为整数，详见支付金额
                        退款货币种类  refund_fee_type      不必填看情况          String(8)       退款货币类型，需与支付一致，或者不填。符合ISO 4217标准的三位字母代码，默认人民币：CNY，其他值列表详见货币类型
                        退款原因    refund_desc            不必填看情况          String(80)      若商户传入，会在下发给用户的退款消息中体现退款原因
                                                                                                                                                                      注意：若订单退款金额≤1元，且属于部分退款，则不会在退款消息中体现退款原因
                        退款资金来源  refund_account       不必填看情况          String(30)      仅针对老资金流商户使用
                                                                     REFUND_SOURCE_UNSETTLED_FUNDS---未结算资金退款（默认使用未结算资金退款）
                                                                     REFUND_SOURCE_RECHARGE_FUNDS---可用余额退款

                        退款结果通知url notify_url        不必填看情况         String(256)     异步接收微信支付退款结果通知的回调地址，通知URL必须为外网可访问的url，不允许带参数
    ...
    public Map<String, String> reverse(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs) throws Exception {
        ...
        String respXml = this.requestWithCert(url, this.cateringRefund(reqData), connectTimeoutMs, readTimeoutMs);
        return this.processResponseXml(respXml);
    }
